function mapEntryRow(row, images) {
  return {
    id: row.id,
    keyword: row.keyword,
...
export function createEntriesRepo(db) {
  ...
  const searchEntriesStmt = db.prepare(`
    SELECT id, keyword, note_text, icon_path, created_at, updated_at
    FROM entries
    WHERE keyword LIKE @term OR note_text LIKE @term
    ORDER BY updated_at DESC
    LIMIT @limit
  `);

  const createEntryTx = db.transaction(({ keyword, noteText, iconPath, images }) => {
  ...
  const updateEntryTx = db.transaction(({ id, keyword, noteText, iconPath, images }) => {
    const now = new Date().toISOString();
  ...
  function createEntry(payload) {
    return createEntryTx(payload);
  }

  function updateEntry(id, payload) {
    return updateEntryTx({ id, ...payload });
  }
